...
- Claim Grounding: 完成宣言（`state/completion-declaration.md`）の主張を Evaluation 結果・根拠リンクと照合する。宣言ファイルが無いときは `skip`（通常の `loop:run` を止めない）。Evaluation 欠落は `stop`、根拠不足は `warn`。UI Polish は観察証拠・ページ枠照合・借り契約・操作観察・観察阻害クリアが必須（§2.14 / §2.15 / §2.16）。見本がライブ URL なのに見本スクショが無い完成も無効。欠落は `stop`。
...
- **対象枠ロック（2026-08-14）**: 対象画面の既存枠が正。見本が専用シェルでも、業務ハブ（`/proposals` 等）の `DashboardLayout` は残す。`SecurityLayout` は文書シェル（`/security`・`/help`）。業務ハブには使わない。原子（`Button` / `Modal` / `Toast`）の再利用とページ枠は別。2026-08-13 の「見本が専用シェルなら DashboardLayout で包まない」は、**対象が文書シェルのときだけ**に限定する。
...
- 関連: `loops/goals/ui-polish-gate.md`, `loops/goals/ui-polish.md`, `scripts/lib/claim-grounding.mjs`, `scripts/lib/context-budget.mjs`, `loops/evals/ui-polish.completion.json`, `.cursor/rules/ui-design.mdc`, `.cursor/rules/agent-loops.mdc`, §2.9, §2.14, §2.16, §6.45, §6.56, §6.59, §10.29, §10.37, §10.38, §10.39
...
- 混同しない: jakubkrehel/skills は Review（§2.11）。desktop-harness は Observe（§2.14）。Nani の情報設計の楽さは本節。ページ枠・対象枠ロックは §2.15。
...
| レセコン身元突合 | `src/contracts/receconIdentity.contract.ts` | 患者の正は `patients.id`。橋は `external_id` → `chart_number`。レセプト伝票ID・氏名だけでは突合しない（§6.58） | CSV取込、ノーザ接続 Adapter |
| レセコン接続安全条件 | `src/contracts/receconIntegration.contract.ts` | いまの経路は CSV。接続は TLS 1.3・許可相手・443・DBポート禁止。既定は参照のみ。準拠済みとは書かない（§6.58） | `/security#rececon`、`/help`、取込 |
| レセコン想定ベンダー | `src/contracts/receconVendor.contract.ts` | 想定は株式会社ノーザ（通称ノーズ）。院向けは「レセコン」。医院名は未確認のまま書かない（§6.58） | 内部契約、`clinics.metadata.rececon_vendor` |
| 監査アセット・保持方針 | `src/contracts/complianceAssets.contract.ts` | ログ5年。種まきは利用中＋終了/削除指示から90日。事故は24時間以内。所在地はシンガポール。署名と突合合格は先送りしない。準拠済みは書かない（§6.66） | `/security/privacy`、`docs/compliance/` |
...
- 2026-08-16 追記: レセコン接続（想定ベンダーはノーザ）も正規化 Adapter 経由。医院LAN非接続は永久禁止ではない。接続条件の正は `receconIntegration.contract`。身元の正は `receconIdentity.contract`。想定ベンダーの正は `receconVendor.contract`。院向けは「レセコン」。医院名は未確認のまま書かない。
...
- **表示切替（2026-08-14 改定）**: `/icon/layouting.png`。表示中はサイドバー上部「デンタクル」の**右隣（行末寄り・幅を空けて）**に置く。非表示時はヘッダー左に同アイコンを出し、押すと再表示。md未満は同アイコンで左ドロワーを開く（ESC・背景クリック・画面遷移で閉じる）。位置・サイズ・開閉は従来どおり。`SecurityLayout` のモバイルメニューは業務サイドバーではないため `/icon/grid.png` のまま。
...
- **見た目**: `source === 'auto_proposal'` かつ `status === 'tentative'` の枠は点線ボーダー（`visitBlockAppearance.ts`）。ラベル例:「仮（クリックで詳細）」。
- **左の濃い緑アクセントバーは置かない**（本予約の `borderLeft` 号車色バーも廃止）。薄い緑背景＋細枠のみ。
- **下端リサイズハンドル**: 常時の緑バーを出さない。通常は透明。ホバー時も目立つ緑／灰バーを出さない（§10.17）。
- **操作（2026-08-14 改定）**: 枠クリックは常に詳細を開く（`visitClickAction.ts`）。仮枠のクリック即確定はやめる。本予約確定は詳細の「本予約に確定する」または一括確定（§6.34）。`confirmTentativeVisit` の確定副作用は維持。
- **詳細からの確定の即時反映**: 先にローカルで `status=confirmed` し、DB更新後は全件 reload しない。電話確認 `pending→ok` は背面同期。
- **クリックとドラッグの分離**: `pointerDown` だけでは移動プレビューを出さない。`moved === true` のときだけ移動中オーバーレイ（§10.15）。
...
- 2026-08-16 追記: `/import` は「当面はAPIの代わり」と書かない。いまのCSV経路と、導入に備えて固定した身元・安全条件を書く。
...
- **共通 Modal の幅（2026-08-14）**: 既定 `md`（`max-w-2xl`）は変えない。広い画面は `lg` / `xl` を足して対象だけ指定する。訪問詳細は `lg`（§6.61）。連絡者リスト用 `xl` までは訪問詳細に使わない。
- **チェック・整数（2026-08-21）**: フラグは共通 `Checkbox`（主色の角丸枠）。設定行のオンオフは従来どおり `Switch`。分数など整数の増減は `NumberStepper`。見た目用のネイティブ `type="number"` スピナーとブラウザ標準チェックは新規に出さない。
- 関連: `DatePicker.tsx`, `TimePicker.tsx`, `Select.tsx`, `Checkbox.tsx`, `NumberStepper.tsx`, `Modal.tsx`, `useAnchoredPopover.ts`, §6.26, §6.51, §6.61, §6.69

### 6.44 患者一覧・電話確認一覧UI（2026-08-09 決定 / 2026-08-14 改定 / 2026-08-21 改定）
...
- **電話確認（2026-08-14）**: 同型の画面内スクロール。件数は `表示 N件 ／ 未完了 N件 ／ 未確認 N件`。状態フィルタは見出し右のコンパクト Select。短い日本語ラベル（未確認・未登録・折返し待ち・施設確認待ち）は `whitespace-nowrap`（§10.41）。既存の OK / NG / 再提案と選択チェックは維持する。
- **検索位置（2026-08-21）**: 当初は電話確認の検索を `titleAside` に置いた。同日再改定で使わない。
- **検索位置（2026-08-21 再改定）**: 「氏名・カルテで検索」は見出し右の `actions`。電話確認は状態 Select の左隣。患者一覧はデータ出力の左隣（新規登録のさらに左ではない）。縦幅は Select `sm` / ボタンと同じ `py-2.5`・`text-sm`。`titleAside` と件数行には置かない。部品は `NameChartSearchInput`。
- 関連: `PatientsPage.tsx`, `PatientsTable.tsx`, `ContactsPage.tsx`, `ContactsTable.tsx`, `NameChartSearchInput.tsx`, §6.31, §6.45, §6.68, §10.41
...
- **斜線でブロック（2026-08-14）**: 空き枠パネルから作成モーダルを `block` で開いてよい。見た目と削除確認は §6.32。
- 関連: `GapFillPanel.tsx`, `runCalendarGapFill.ts`, `calendarGapFillError.ts`, `api/schedule/gap-fill.ts`, `runGapFillJob.ts`, `rankGapFillByProximity.ts`, `buildGapFillSnapshot.ts`, `buildGapFillPrompt.ts`, `createTentativeAutoProposal`, `ComposingOrb.tsx`, §6.5, §6.11, §6.32, §6.34, §6.39, §6.40
...
- **切替（2026-08-14）**: 見出しは常に「設定」。右端に灰トラック＋選中白ピル（`SegmentedControl`）で **導入タイプ / メニュー / チーム / 担当 / 稼働枠**（5択）。見出し右 Select「表示」とテキストタブ表記には戻さない。Proバッジとキラキラは借りない。4択に戻さない。長い説明文は置かない。メニューの中身は §6.60。
...
### 6.55 お知らせ（プロダクト更新・2026-08-13 決定 / 2026-08-14 追記 / 同日ご意見返答追記 / 同日画面改定）

- 正本はログイン後 `/announcements`。院も運営も同じ入口。
- 入口はアカウントメニュー（マイページの次）。業務ナビ（サイドバー）と設定（導入タイプ・メニュー・チーム・担当・稼働枠）には置かない。
- ログイン画面には更新内容を出さない（§6.21）。
- `/proposals` は AI スケジュール自動提案。お知らせと画面名・URL・メニューを混ぜない（§6.34）。
- コード修正・デプロイだけでは院向けお知らせに出さない。
- 流れ: 新規は必ず `propose`（`proposed`）。公開は `publish`（`published`）。入れないは `reject`（`rejected`）。院に見えるのは入れる後だけ。いきなり `published` で作らない。
- 書込は RPC のみ。作成・公開・却下は `propose_product_update` / `publish_product_update` / `reject_product_update`。追加（2026-08-14）: `update_product_update_copy` / `delete_product_update` / `set_product_update_in_progress_badge` / `set_product_update_timeline_mark`。既存3つのシグネチャは変えない。テーブル直書き禁止。公開判定の SSoT は `productUpdatePolicy.ts`。
- エージェントは提案まで。`publish_product_update`（入れる）は運営の人間が行う。進捗の反映済み連動も propose→publish のまま。
- **運営の2入口（2026-08-14）**: 見出し右（`DashboardLayout` の `actions`）に「リリース予定を登録」「更新情報の登録」。斜線カード内には置かない。院には出さない。ボタンは淡い緑面＋主色文字。全体の Button primary と主色 `#008C01` の塗りは変えない。
- **リリース予定（2026-08-14）**: `proposed` をチップ一覧。斜線面は Nani changelog（135deg、白1.5px、透明3px、地 `#EBF6FF`）。見出し左は `/icon/optimization.png`。登録は項目名だけ。未指定は既存デフォルト（feature/web/空）で `propose`。入れる／入れないは予定パネルに出さない。
- **開発中バッジ（2026-08-14）**: 予定チップの右上外側・約8度。文言は「開発中」。英語 WIP / beta / 国旗は借りない。正は `show_in_progress_badge`（初期 ON）。切替は Switch。公開チップには付けない。
...
- **削除（2026-08-14）**: `improvement_items.product_update_id` がある件は削除しない。反映済みを外してもお知らせは消さない。
- **version（2026-08-14）**: `product_updates` の UPDATE には `version` 列が必要。共通 `set_updated_at` が増やす。列が無いと入れる／入れない／開発中切替が落ちる（§10.48）。
...
- 関連: `AnnouncementsPage.tsx`, `productUpdatePolicy.ts`, `formatProductUpdate.ts`, `productUpdateMark.ts`, `AccountMenu.tsx`, `product_updates`, `propose_product_update`, `set_improvement_item_status`, `.cursor/commands/issues.md`, §6.21, §6.24, §6.33, §6.34, §6.54, §6.57, §10.48, §10.49
...
- 2026-08-16 追記: `/security#rececon` はいまのCSVと、導入に備えた接続条件の両方を書く。ベンダー製品名は出さない。
- 2026-08-14 追記: 同じ枠を `/help` でも使う（§6.59）。左レールにヘルプを置く。ヘルプ本文は `surface="plain"`（白 article で包まない）。
- 関連: `SecurityLayout.tsx`, `SecurityRail.tsx`, `securityCopy.ts`, `placeAccountMenu.ts`, `AccountMenu.tsx`, §6.24, §6.45, §6.58, §6.59, §10.31
...
### 6.58 レセコン連携方針（案A・2026-08-14 決定 / 2026-08-16 改定）
...
#### 2026-08-16 改定（今から備える。CSV非接続を思想の正にしない）

- 「当面はCSVだけ」は、**いま開いている経路**の説明である。ノーザ導入を長期の宿題にしない。接続条件・身元受け皿・院向け説明は今から固定する。
- 想定レセコンは **株式会社ノーザ**（通称ノーズ）。院向け画面・ヘルプ・取込説明は「レセコン」と書く。WiseStaff / clevia / ノーザ / ノーズは院向けに出さない。
- 医院名は未確認のまま MEMORY に書かない。
- いま開いている経路: `clinic_csv_export`。接続モードの正は `src/contracts/receconIntegration.contract.ts`。
- 患者身元の正は `patients.id`。橋は `patients.external_id`（安定患者ID）→ `patients.chart_number`（カルテ番号）。レセプト伝票IDと氏名だけでは突合しない。正は `src/contracts/receconIdentity.contract.ts`。
- 想定ベンダーの正は `src/contracts/receconVendor.contract.ts`。院 metadata の受け皿は `clinics.metadata.rececon_vendor`（未設定は `nhosa`）。
- 本番DBに `clinics.external_code` と `patients.external_id` を入れ済み。`external_id` にカルテ番号をコピーしない。
- 今の対策として守る5項目:
  - 通信: TLS 1.3、許可した相手以外遮断、開けるポートは 443、DBポート禁止。エージェント直結禁止。
  - 最小権限: 既定は参照（取込）。書き込みは別契約。取る列はカルテ番号・外部ID・氏名・担当医・最終来院日・診療回数。生年月日・保険証・カルテ本文・レセプト・点数・請求額は取らない。
  - 暗号化・保持: 鍵は画面と AI に渡さない。CSVは取込後に残さない。種まきは業務データとして残す。ログは5年（§6.66）。
  - 責任分界: 医院LAN内側とレセコン本体は医院／事業者。取込後の保存・画面・権限・スケジュールはデンタクル。VPN／閉域の医院側終端は医院、クラウド側終端はデンタクル。
  - ガイドライン: 厚労省『医療情報システムの安全管理に関するガイドライン』と経産・総務の事業者向けガイドラインを参照して取り入れる。「3省2」だけを正にしない。監査前に準拠済みとは書かない。
- ライブAPIクライアントとベンダー鍵の保存は、接続情報を受け取ってから開く。条件と受け皿は今ある。
- 監視SaaSなし、取込監査は件数・成否だけ、エージェントのレセコン直結禁止は維持する。
- 院向け説明の正は `/security#rececon`。いまのCSVと、導入に備えた接続条件の両方を書く。

### 6.59 ヘルプセンター（`/help`・2026-08-14 決定）

`/help` は文書シェル。業務ダッシュボードの内側パネル寄せにしない。枠は安全性と同じ `SecurityLayout`（§6.56）。本文は FAQ 井戸をキャンバスに置くため `surface="plain"`。安全性の白 article で包まない。

- 入口: アカウントメニュー（安全性の次）、左レール、フッター「サポート」。サイドバーとログイン画面には置かない（§6.24）。
- レール幅・固定・本文 `max-w-4xl`・アカウントメニュー上開きは §6.56 と同じ。
- FAQ のグループ見出し（「よくある質問」等）は角丸井戸（薄い緑 `#DCEFDD`）の**内側**。見出しを井戸の外に置かない（§10.45）。
- 左右のふち（井戸と白カードの隙間）は細い（`px-1.5`）。`p-5` で左右を広げない。
- 文言の正は `helpCopy.ts`。項目追加は `HELP_SECTIONS` に足す。Nani の FAQ 本文・製品名は借りない。事実はデンタクル。
...
- `staffId` を渡したときだけ担当を更新する。未指定では既存担当を消さない。
- 関連: `VisitDetailModal.tsx`, `VisitBriefingSection.tsx`, `VisitReservationTable.tsx`, `useCalendarVisitActions.ts`, §6.6, §6.32, §6.35, §6.43, §6.60, §10.50, §10.51

### 6.62 ログイン後シェル固定（2026-08-14 決定）
...
### 6.66 監査方針の固定と残作業は先送りしない（2026-08-16 決定）

準拠済みの対外表示はまだ禁止。次の数字は方針として今から守る。未決に戻さない。

- ログイン監査・操作ログの保存: **5年**。改ざん防止。運営のみ閲覧。
- 患者種まき: 利用中は保持。契約終了または削除指示から **90日以内** に削除。診療録の法定保存と混ぜない。
- 事故の医院管理者への連絡: 覚知から **24時間以内**。
- データ所在地: Supabase `ap-southeast-1`（シンガポール）。隠さない。
- 今やる残作業（先送り禁止）: 国外保存の医院管理者署名、データ処理契約の締結、ガイドライン突合の承認者・日付。
- 院向け正は `/security/privacy`。監査文書の正は `docs/compliance/` と `complianceAssets.contract.ts`。
- ログイン画面に同意チェックを置かない。医院が利用目的を決め、Dentacle は受託する。
- 関連: §6.12, §6.15, §6.56, §6.58, §10.62

### 6.67 マイページ本人編集（2026-08-21 決定）

`/mypage` で本人が直せるのは表示名だけ。

- 編集対象は `profiles.display_name` のみ。
- メールアドレスはログインIDとして表示のみ。`auth.updateUser({ email })` と `profiles.email` の更新はしない。
- 所属クリニックは表示のみ。付け替えはユーザー管理（§6.25）。マイページから `clinic_members` を変えない。
- 保存列は `display_name` と `updated_at` のみ。
- パスワード変更はこの画面に置いていない（後続可）。
- 関連: `MyPage.tsx`, `useMyProfile.ts`, `myProfilePolicy.ts`, §6.24, §6.25, §6.63

### 6.68 患者アイコン（2026-08-21 決定）

患者一覧・電話確認のお名前左と、詳細・新規登録で出す丸画像。

- 正は `patients.metadata.user_icon`（`"1"`〜`"7"`）。画像は `public/user_icon/{n}.png`。新カラムは作らない。
- 未設定は患者IDの安定ハッシュで表示だけ割り当てる。一括 UPDATE しない。
- 詳細の基本情報と新規登録で変更できる。カレンダーには出さない。
- **電話確認（2026-08-21）**: お名前左も同じ `PatientIcon`。判定は `resolvePatientIconId` / `toContactRow`。カレンダーの電話ポップオーバーには出さない。
- 保存と CSV 再取込は metadata をマージする。`visit_count` を消さない。
- コンポーネントは `PatientIcon.tsx`、判定は `patientIconPolicy.ts`。同一フォルダで語幹が大文字小文字だけ違うファイルを置かない（§10.63）。
- 関連: `patientIconPolicy.ts`, `PatientsTable.tsx`, `ContactsTable.tsx`, `mapContactRow.ts`, `PatientDay0Form.tsx`, `importPatientCsv.ts`, §6.44, §10.64

### 6.69 患者詳細の基本情報・訪問条件UI（2026-08-21 決定）

`PatientDay0Form` の入力幅と独自部品。

- 短い項目（主担当・頻度・日付・所要・時刻）は内容幅で wrap する。`md:grid-cols-2` で半分幅まで伸ばさない。
- 住所だけ全幅。希望終了時刻を半行で残さない。
...
- レセコン接続は今から備える。DBポートを開かない。鍵を画面・AIに渡さない。`external_id` にカルテ番号をコピーしない（§6.58）
- 監査ログは5年。種まきは終了/削除指示から90日以内に削除。事故は24時間以内に医院へ連絡。国外保存の署名とDPA締結と突合合格は先送りしない。準拠済みは書かない（§6.66）
...
| 2026-08-14 | FAQ見出しを井戸の外に置いた | セクション見出しを井戸の外に置くと見本のふちが消える | グループ見出しは井戸の内側。左右ふちは細い（§6.59 / §10.45） |
| 2026-08-14 | 使っている import を未使用として消した | JSX の一部だけ見て消した | 全使用箇所を Grep してから消す（§10.46） |
| 2026-08-14 | 業務枠が最上部でゴムバンドする | 非 fillViewport に `min-h-screen` | 業務枠ルートに `min-h-screen` を使わない（§6.62 / §10.47） |
| 2026-08-14 | お知らせ UPDATE が version 欠落で落ちる | `set_updated_at` が増やす列が無かった | `product_updates.version` を維持（§6.55 / §10.48） |
...
### 10.40 観察で重複・FAB衝突を書いて完成にした（2026-08-13）
...
### 10.45 FAQ見出しを井戸の外に置いた（2026-08-14）

- 事象: Nani ヘルプ見本では「よくある質問」が角丸井戸の内側にあり、ふちがタイトルを囲む。実装は見出しを井戸の外に置いたためふちが消えた。
- 原因: セクション見出しを白カードの外・井戸の外に置くと見本の井戸形にならない、と誤った。
- 再発防止: グループ見出しは井戸（薄い緑の角丸）の内側。左右ふちは細く保つ（`px-1.5`）。`p-5` で左右を広げない。
- 関連: `HelpFaqSection.tsx`, `/help`, §6.59

### 10.46 使っている import を未使用として消さない（2026-08-14）

- 事象: 担当 Select を Briefing へ移したあと、`VisitDetailModal` の Select import を未使用として消した。号車 Select が残っていたため実行時に壊れた。
- 原因: 移した箇所だけ見て、残りの JSX を Grep しなかった。
- 再発防止: 未使用判定は JSX の全使用箇所を Grep してから。
- 関連: `VisitDetailModal.tsx`, §6.61

### 10.47 業務枠ルートに min-h-screen を使わない（2026-08-14）

- 事象: `DashboardLayout` の非 fillViewport が `min-h-screen` だとヘッダーとサイドバーが本文と一緒に流れ、最上部でゴムバンドする。
- 原因: 文書ページの高さ確保を業務枠ルートに使った。
- 再発防止: ログイン後シェルは §6.62。`min-h-screen` は業務枠ルートに使わない。
- 関連: `DashboardLayout.tsx`, `appShell.ts`, §6.62, §10.9

### 10.48 product_updates の UPDATE には version 列が必要（2026-08-14）

- 事象: 入れる／入れない／開発中切替が `record has no field version` で落ちた。
- 原因: 共通 `set_updated_at` が `new.version` を増やすのに列が無かった。
- 再発防止: `product_updates` を UPDATE する RPC を足すとき `version` を落とさない。
- 関連: `product_updates.version`, `set_updated_at`, §6.55

### 10.49 news アイコンの表示パスは実在ファイルと揃える（2026-08-14）

- 事象: file-text を link に差し替えたあと、表示パスが古いままで文書アイコンが欠けた。
- 原因: DBキーや旧ファイル名のまま src を残した。
- 再発防止: `productUpdateMarkSrc` のパスは `public/icon/news` の実在ファイルと揃える。
- 関連: `productUpdateMark.ts`, `public/icon/news`, §6.55

### 10.50 予約キャンセルは画面内確認（2026-08-14）

- 事象: 訪問詳細のキャンセルをブラウザ `confirm` にすると業務UIの階層が崩れる。
- 原因: 短い確認を OS ダイアログに任せた。
- 再発防止: `VisitCancelConfirm` を使う。`window.confirm` に戻さない。
- 関連: `VisitCancelConfirm.tsx`, `VisitDetailModal.tsx`, §6.61

### 10.51 訪問詳細の時刻は2列、メニューと色は全幅（2026-08-14）

- 事象: 詳細モーダルを広げただけでは号車・開始・終了が1列のまま余白が空く。
- 原因: 外枠の `lg` だけ変えて、中のグリッドを作成モーダルと揃えなかった。
...
### 10.61 CSV非接続を永久思想にしてノーザ導入を先送りした（2026-08-16）

- 事象: 「医院LANに入らない」「当面はCSVだけ」「製品名は出さない」を思想の正にすると、ノーザ導入の受け皿と安全条件まで先送りになる。
- 原因: いま開いている経路（CSV）と、今から固定すべき接続契約を混ぜた。
- 再発防止: CSVは現経路。想定ベンダーはノーザ（院向けはレセコン）。接続5項目と身元突合は今から契約として守る。ライブAPIは接続情報を受けてから。医院名は未確認のまま書かない。
- 関連: §6.12, §6.20, §6.42, §6.58, `receconVendor.contract.ts`, `receconIntegration.contract.ts`, `receconIdentity.contract.ts`

### 10.62 監査の年数・署名を未決のまま先送りした（2026-08-16）

- 事象: ログ年数・削除期限・報告期限を「未決」のまま置くと、準拠済み以前の監査アセットまで止まる。
- 原因: 推奨案を方針にせず、署名作業まで全部未決扱いにした。
- 再発防止: 5年 / 90日 / 24時間は方針。国外同意・DPA署名・突合合格は今やる残作業。準拠済みは残作業後だけ。
- 関連: §6.66, `complianceAssets.contract.ts`, `docs/compliance/未決事項.md`

### 10.63 同一フォルダの大文字小文字衝突（2026-08-21）

- 事象: `PatientIcon.tsx` と `patientIcon.ts` を同じディレクトリに置いた。Vite が `patientIcon.ts` を読み、`PatientIcon` の export が無いと実行時エラーになった。`tsc` は通った。
- 原因: macOS はファイル名の大文字小文字を区別しない。型チェックを Vite の import 解決の証拠にした。画面の実行時エラーを見ていなかった。
- 再発防止: 同一フォルダで大文字小文字だけ違うファイル名を置かない。コンポーネントとユーティリティは語幹を変える。`tsc` 通過を Vite 解決の証拠にしない。新規 import のあとページを開いてコンソールを見る。
- 関連: `PatientIcon.tsx`, `patientIconPolicy.ts`, `PatientsTable.tsx`, §6.68

### 10.64 患者アイコンを電話確認一覧に出さなかった（2026-08-21）

- 事象: 患者一覧だけ `PatientIcon` にし、同型の電話確認はイニシャル丸のまま残った。
- 原因: お名前左の顔を変えるとき、同型画面の呼び出し元を列挙しきれていなかった。
...
- `loops/goals/ui-polish-gate.md` / `loops/goals/ui-polish.md` / `loops/graphs/ui-polish.mmd` — UI Polish 完成ゲート（Interface Review・§2.11 / 観察証拠・操作観察・§2.14 / ページ枠照合・借り契約・§2.15 / Overlay・観察阻害クリア・§2.16）
...
- `scripts/lib/claim-grounding.mjs` — Claim Grounding（主張↔根拠。UI Polish は観察証拠・ページ枠照合・借り契約・操作観察・観察阻害クリア必須・§2.14 / §2.15 / §2.16。ライブ URL 見本はページ全体スクショ必須・§10.39）
...
- `src/contracts/receconIdentity.contract.ts` — レセコン患者身元（`external_id` → `chart_number`。§6.58）
- `src/contracts/receconIntegration.contract.ts` — レセコン接続の安全条件（5項目。§6.58）
- `src/contracts/receconVendor.contract.ts` — 想定ベンダーはノーザ。院向けはレセコン（§6.58）
- `src/contracts/complianceAssets.contract.ts` — 監査方針（5年 / 90日 / 24時間）と委託先（§6.66）
- `docs/compliance/` — 処理契約案・突合・削除・事故・残作業（§6.66）
- `src/pages/Security/privacyCopy.ts` — 個人情報の取り扱い（§6.66）
...
□ UI Polish なら §2.11（Interface Review `/better-interface`。Verdict が Block なら完成報告禁止）と §2.14（観察証拠: snapshot|screenshot を Read して差分1行。端の▼は開いてから撮る。ライブ URL 見本はページ全体スクショ必須。観察で残した阻害: なし）と §2.15（ページ枠照合・参照の正体は Nani!?・対象枠ロック・借り契約・操作観察。内側パネルだけでは完成無効。業務ハブの DashboardLayout は残す。見本の下開き・298px は借りない。ライブ URL を「なし（指示のみ）」や URL 文字列のまま完成にしない）と §2.16（Overlay / Chat 検査。FAB実装は UI Polish 併用。見出し1箇所・注意非重複・FAB非衝突）を守った
□ 完成報告なら §2.10（Claim Grounding / Working Graph。宣言は state/completion-declaration.md）を守った。UI Polish なら観察証拠欠落・ページ枠照合欠落・借り契約欠落・操作観察欠落・観察阻害未解消・ライブ URL なのに見本スクショ欠落で stop（§2.14 / §2.15 / §2.16 / §10.39 / §10.40）
...
□ レセコン連携なら §6.58（いまの経路はCSV。ノーザ導入は今から備える。接続5項目と身元受け皿を先に固定。ライブAPIは接続情報を受けてから。監視SaaSは足さない。取込監査は operation_traces に件数・成否だけ。院向けは /security#rececon で「レセコン」。製品名は院向けに出さない。院名は未確認のまま MEMORY に書かない）を守った
□ 監査方針なら §6.66 / §10.62（ログ5年。種まきは終了/削除指示から90日。事故は24時間以内。国外同意・DPA・突合合格は先送りしない。準拠済みは書かない。ログインに同意UIを置かない）を守った
...
□ マイページなら §6.67（表示名だけ編集。メール・所属は表示のみ。email / clinic_id / updateUser を送らない）を守った
...
□ チェック・整数入力なら §6.43 / §6.69（フラグは Checkbox。設定行オンオフは Switch。分数は NumberStepper。ネイティブ number スピナーとブラウザ標準チェックを増やさない）を守った
...
- `2026-08-16`: レセコンはノーザ導入を今から備える、に §6.58 を改定。CSVは現経路であり思想の正ではない。接続5項目・身元・想定ベンダー契約を §3 / §6.12 / §6.20 / §6.42 / §6.56 / §7 / §10.61 / §11 / §12 に追記（`/project-memory-learn`）
- `2026-08-16`: 監査方針を §6.66 に固定。ログ5年・種まき90日・事故24時間。国外同意・DPA・突合合格は先送りしない。§3 / §7 / §10.62 / §11 / §12（`/project-memory-learn`）
