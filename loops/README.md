...
- 正式適用:
  ...
  - UI Polish: `loops/goals/ui-polish-gate.md` / `loops/goals/ui-polish.md` / `loops/graphs/ui-polish.mmd`（`maxIterations: 3`）。判定ノードで `/better-interface`（`.cursor/skills/better-interface`）を回す。ページ枠照合・骨格照合なしの完成は無効。ライブURLは見本スクショ必須
...
- Claim Grounding: 完成宣言（`state/completion-declaration.md`）の主張を根拠と照合。無ければ skip。UI Polish は観察証拠（snapshot|screenshot を Read した記録）とページ枠照合と骨格照合が必須。ライブURLは見本スクショ必須。
