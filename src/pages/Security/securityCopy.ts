...
export const SECURITY_SECTIONS: SecuritySection[] = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      '将来レセコンと接続する場合は、医院LANに入らないことが前提ではありません。VPN（IPsecなど）や専用の閉域網、TLS 1.3以上の暗号化通信のうち、許可した通り道だけを使います。',
      ...
      '医院内のレセコン本体・院内PC・CSV書き出しは医院（およびレセコン事業者）の範囲です。取込後の保存・画面・権限・訪問スケジュールはデンタクルの範囲です。将来のVPN／閉域では、医院側終端は医院、クラウド側終端はデンタクルです。',
    ...
    callout: {
      title: '取込画面',
      body: 'いまは患者CSV取込から、院内ファイルを選択して種まきできます。将来のレセコン接続は、この画面とは別に、許可した通り道だけを開きます。',
  ...
  },
  {
    ...
    paragraphs: [
      '画面とサーバーの通信は HTTPS で暗号化します。将来のレセコン接続は TLS 1.3 以上を条件にします。',
...
export const SECURITY_NETWORK_NOTE =
  '自動提案とご意見の外部連携はサーバー側で行います。院内ブラウザの許可リストへ追加する必要はありません。いまのレセコンCSV取込は院内ファイルを画面から送ります。将来のレセコン接続（VPN・閉域・TLS）は、このブラウザ許可リストとは別に、許可した通り道だけを開きます。'
...
export const SECURITY_RAIL_BLURBS = [
  ...
  'いまのレセコン取込はCSVの種まきです。将来の接続は許可した通り道だけを使います。',
] as const
