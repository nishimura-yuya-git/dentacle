...
症状ゲートは `loops/goals/symptom-gate.md` を正とする（must）。ユーザーの言葉のままの症状、前後の数値測定、同一環境、実差分が揃うまで完成にしない。
...
0. 症状ゲート（`loops/goals/symptom-gate.md`）を満たしている。症状をユーザーの言葉のまま残し、前後の数値で測定し、ユーザーと同じ環境で確認し、実差分がある。
...
- 症状を技術的な代理症状に言い換えた（例: 「配置がガクガク」を `ReferenceError` に置き換えた）。
- 「エラーが出ない」「テストが通った」を症状の解消根拠にした（不在証明）。
- 「修正した」と書いたのに変更ファイルが無い。
- ユーザーと違う URL・ポートの結果で、ユーザーの報告を否定した。
...
- iteration: N / 3
- 症状: …（ユーザーの言葉のまま。技術用語に言い換えない）
- 症状測定: …（修正前の数値 → 修正後の数値。「エラーなし」は不可）
- 確認環境: …（ユーザーと同じ URL・ポート）
- 完成条件: 0□ 1□ 2□ 3□ 4□ 5□ 6□ 7□（満たした番号を明示）
- Evaluation:
  - コマンド: …
  - 結果: pass / warn / stop
  - warn の根拠: …（warn 時のみ）
