...
export function provisionalStatusLabel(heightPx: number): string {
  return heightPx < PROVISIONAL_BLOCK_MIN_HEIGHT_PX ? '仮' : '仮（クリックで確定）'
}
...
export function visitBlockClassName(visit: {
  ...
  if (isAutoProposalTentative(visit)) {
    return [
      ...
      'border-2 border-dashed border-[#008C01]/55 bg-[#E8F5E9] px-1.5 py-1 text-left',
  ...
  return [
    ...
    'border border-[#008C01]/30 bg-[#E8F5E9] px-1.5 py-1 text-left shadow-sm',
