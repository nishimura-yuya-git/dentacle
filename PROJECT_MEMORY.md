...
- **既決定の扱い（2026-08-13）**: 「やるか」と聞かれても S-02 / S-07 を任意扱いにしない。決定文を先に返す。優先度の再評価は人間が Memory を改正するときだけ（§10.29 / §10.30）。
- 関連: Advisors、§6.40、§7
...
| 2026-08-13 | §6.52 の既決定を AI 意見で再優先度づけした | 読んだあと一般論で立法を上書きした | 既決定は決定として先に返す。優先度の再評価はしない（§10.29 / §10.30） |
...
### 10.29 プラットフォーム保安の既決定を AI 意見で再優先度づけしない（2026-08-13）

- 事象: 漏洩パスワード保護 / DB SSL Enforcement / PITR を「やった方がいいか」と聞かれ、§6.52・§7 を読んだうえで S-07 を「急がない」と返した。
- 原因: 立法 SSoT の決定文より、一般的なリスク論・課金論で優先度を付け直した。
- 再発防止: S-02 漏洩PW ON・S-07 SSL Enforcement ON・S-09 PITR は課金承認後、は **2026-08-12 の恒久方針**。AI はこれを「やるか」の再評価対象にしない。仕組みの説明はよいが、必須を任意に下げない。
- 関連: §6.52, §7, `.cursor/rules/constitution.mdc`

### 10.30 既決定の保安項目は「やるか」でも決定内容を先に返す（2026-08-13）

- 事象: 開いた技術相談として優先度表を出し、案件の正（§6.52 / §7 必須）より先に私見を置いた。
- 原因: 質問の表面（是非）を、立法済みかどうかより先に扱った。
- 再発防止: 「やった方がいいか」でも Memory に決定がある項目は、先に該当 § と決定文を返す。再評価が必要なら人間が Memory を改正する。
- 関連: §6.52, §7
...
□ 既決定の保安を「やるか」と聞かれても §6.52 / §7 の決定を先に返す。AI意見で再優先度づけしない（§10.29 / §10.30）
...
- `2026-08-13`: プラットフォーム保安の既決定を AI 意見で再優先度づけしない再発防止を §6.52 / §10.29 / §10.30 / §12 に追記（`/project-memory-learn`）
